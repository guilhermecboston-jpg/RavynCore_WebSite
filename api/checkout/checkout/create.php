...
try {
    ...
    $ins = $pdo->prepare('INSERT INTO ravyn_checkout_orders
    ...
    checkout_json_response(['error' => 'Erro ao registrar pedido'], 500);
}
...
if ($gateway === 'mercadopago') {
    ...
    if ($token === '') {
        checkout_json_response(['error' => 'Mercado Pago não configurado (MP_ACCESS_TOKEN)'], 503);
    }
    ...
    if ($httpCode < 200 || $httpCode >= 300 || empty($data['init_point'])) {
        checkout_json_response(['error' => 'Falha ao criar preferência Mercado Pago'], 502);
    }

    if (!empty($data['id'])) {
        $pdo->prepare('UPDATE ravyn_checkout_orders SET gateway_ref = ? WHERE id = ?')
...
if ($stripeKey === '') {
    checkout_json_response(['error' => 'Stripe não configurado (STRIPE_SECRET_KEY)'], 503);
}
