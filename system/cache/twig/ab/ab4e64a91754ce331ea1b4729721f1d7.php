...
\t\t\$('.rc-password-toggle').on('click', function () {
...
\tfunction ensureAccountValue()
\t{
\t\tvar \$account = \$('#account_input');
\t\tif(!\$account.length) {
\t\t\treturn;
\t\t}

\t\tvar currentValue = (\$account.val() || '').trim();
\t\tif(currentValue.length) {
\t\t\treturn;
\t\t}

\t\tvar maxLength = parseInt(\$account.attr('maxlength') || '30', 10);
\t\tvar emailValue = (\$('#email').val() || '').trim().toLowerCase();
\t\tvar baseName = '';
\t\tif(emailValue.indexOf('@') > 0) {
\t\t\tbaseName = emailValue.split('@')[0].replace(/[^a-z0-9]/g, '');
\t\t}

\t\tif(";
        // line 163
        echo ((twig_constant("USE_ACCOUNT_NAME")) ? ("true") : ("false"));
        echo ") {
\t\t\tvar fallbackName = ('rc' + Date.now().toString()).substring(0, maxLength);
\t\t\t\$account.val((baseName || fallbackName).substring(0, maxLength));
\t\t}
\t\telse {
\t\t\tvar digits = Date.now().toString().replace(/\\D/g, '');
\t\t\t\$account.val(digits.substring(Math.max(0, digits.length - Math.min(maxLength, 10))));
\t\t}
\t}
